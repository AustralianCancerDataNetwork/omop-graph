...
def populate_reference_data(
    ...
    location_data = [
        {
            "location_id": loc_ids.next(),
            "country_concept_id": choice(avail_country),
            "city": f"City {idx}",
        }
        for idx in range(10)
    ]
    ...
    care_site_data = [
        {
            "care_site_id": cs_ids.next(),
            "care_site_name": f"Care Site {idx}",
            "location_id": choice(locations).location_id,
    ...
    provider_data = [
        {
            "provider_id": pro_ids.next(),
            "specialty_concept_id": choice(avail_provider),
            "gender_concept_id": choice(avail_gender),
            "care_site_id": choice(care_sites).care_site_id,
        }
        for _ in range(50)
    ]
...
def populate_observation_periods(
    ...
    rows = (
        ...
        .join(
            Observation_Period,
            Observation_Period.person_id == Visit_Occurrence.person_id,
            isouter=True,
        )
...
def populate_conditions_and_modifiers(
    ...
    rows = (
        session.query(Observation_Period, Death, Condition_Occurrence)
        .join(Death, Observation_Period.person_id == Death.person_id, isouter=True)
        .join(
            Condition_Occurrence,
            Observation_Period.person_id == Condition_Occurrence.person_id,
            isouter=True,
        )
        .all()
    )
...
def populate_test_data(session):
    ...
    concept_by_domain = pd.DataFrame(
        session.query(*Concept.__table__.columns).filter(
            sa.or_(
                ...
                sa.and_(
                    Concept.domain_id == "Condition", Concept.vocabulary_id == "ICDO3"
                ),
            )
        )
    )
    ...
    avail_gender = list(
        concept_by_domain[concept_by_domain.domain_id == "Gender"].concept_id
    )
    avail_ethnicity = list(
        concept_by_domain[concept_by_domain.domain_id == "Ethnicity"].concept_id
    )
    avail_race = list(
        concept_by_domain[concept_by_domain.domain_id == "Race"].concept_id
    )
    avail_place_of_service = list(
        concept_by_domain[concept_by_domain.domain_id == "Visit"].concept_id
    )
    avail_country = list(
        concept_by_domain[concept_by_domain.concept_class_id == "Location"].concept_id
    )
    avail_provider = list(
        concept_by_domain[concept_by_domain.domain_id == "Provider"].concept_id
    )
    avail_types = list(
        concept_by_domain[concept_by_domain.domain_id == "Type Concept"].concept_id
    )
    cancers = list(
        concept_by_domain[
            (concept_by_domain.domain_id == "Condition")
            & (concept_by_domain.vocabulary_id == "ICDO3")
            & (concept_by_domain.concept_code.str.contains("/3"))
        ].concept_id
    )
    ...
    populate_reference_data(
        session, avail_country, avail_place_of_service, avail_provider, avail_gender
    )
    session.commit()
    care_sites = session.query(Care_Site).all()

    # People and visits
    populate_people_and_visits(
        session,
        care_sites,
        avail_gender,
        avail_race,
